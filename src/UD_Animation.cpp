...
std::vector<int> UD::AnimationManager::GetAllAnimationFilesStatus() const
{
    ...
    for (auto&& [name,file] : _jsoncache)
    {
        loc_res.push_back((int)file.status);
    }
...
std::vector<std::string> UD::AnimationManager::GetAnimationsFromJSON(std::string a_def, std::vector<RE::Actor*> a_actors, int a_constraintsORA1, int a_constraintsORA2)
{
    ...
    if (loc_it->second.status != AnimationFileStatus::sOK) return {"ERROR",std::format("JSON {} is disabled",loc_filename)}; 
    ...
    for (int i = 0; i < a_actors.size(); i++)
    {
        ...
        auto loc_A = JSONUtility::GetSingleton()->RecursiveFind(loc_object,loc_anim_var_path);
    ...
    //bool is_sequence = JSONUtility::GetSingleton()->GetPathStringValue(file, actor_animVars[0] + ".anim[0]", "") != ""
    
    std::vector<std::string> loc_res = {"OK"};
    if (a_actors.size() == 2U)
    {
        auto loc_anim1 = JSONUtility::GetSingleton()->RecursiveFind(loc_object, loc_actor_animVars[0] + ".anim");
        auto loc_anim2 = JSONUtility::GetSingleton()->RecursiveFind(loc_object, loc_actor_animVars[1] + ".anim");
    ...
    {
        auto loc_anim = JSONUtility::GetSingleton()->RecursiveFind(loc_object, loc_actor_animVars[0] + ".anim");
...
std::vector<std::string> UD::AnimationManager::GetAnimationsFromDB(std::string a_type, const std::vector<std::string>& a_kws, std::string a_field, const std::vector<int>& a_ActorConstraints, int a_lewdmin, int a_lewdmax, int a_aggromin, int a_aggromax)
{
    ...
    for (auto&& kw : a_kws)
    {
        ...
        {
            if (animfile.status != AnimationFileStatus::sOK) continue;
    
            auto loc_obj = animfile.json->as_object();
            auto loc_type = JSONUtility::GetSingleton()->RecursiveFind(loc_obj,loc_dict_path);
            ...
            if (loc_type.is_array())
            {
                ...
                for (auto&& anim : loc_type.as_array())
                {
                    ...
                    {
                        auto loc_A = JSONUtility::GetSingleton()->RecursiveFind(anim,"A" + std::to_string(i+1));
                        if (loc_A.is_array())
                        {
...
void UD::AnimationManager::SyncAnimationSetting(const std::vector<std::string>& a_animationoff)
{
    ...
    for (auto&& it : a_animationoff)
    {
        ...
        {
            loc_find->second.status = AnimationFileStatus::sDisabled;
...
void UD::AnimationManager::Setup()
{
    if (!_init)
    {
        ...
        std::string loc_animpath = RelToAbsPath("skse\\plugins\\StorageUtilData\\UD\\Animations");
        ...
        for (const auto & entry : std::filesystem::directory_iterator(loc_animpath))
        {
            ...
            if (entry.is_regular_file() && std::regex_match(loc_path,loc_regex)) 
            {
                ...
                _jsoncache[loc_name] = AnimationFile{std::shared_ptr<boost::json::value>(new boost::json::value(loc_json)),AnimationFileStatus::sOK,"OK"};
    
                // Check json status
                auto loc_jsoncond   = JSONUtility::GetSingleton()->RecursiveFind(loc_json.as_object(),"conditions.json");
                auto loc_modcond    = JSONUtility::GetSingleton()->RecursiveFind(loc_json.as_object(),"conditions.mod");
                ...
                if (loc_jsoncondstr != "")
                {
                    const std::string loc_condjsonpath = RelToAbsPath("skse\\plugins\\StorageUtilData\\UD\\Animations" + loc_jsoncondstr);
                ...
                if (!loc_cond)
                {
                    _jsoncache[loc_name].status = AnimationFileStatus::sMissingMaster;
                }
            }
        }
...
bool UD::AnimationManager::_CheckConstraints(boost::json::value a_obj, std::string a_ObjPath, int a_ActorConstraints) const
{
    ...
    try
    {
        auto req = JSONUtility::GetSingleton()->RecursiveFind(a_obj,a_ObjPath + ".req");
        ...
        {
            auto opt = JSONUtility::GetSingleton()->RecursiveFind(a_obj,a_ObjPath + ".opt");
